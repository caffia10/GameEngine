...
inline GLuint CreateShader(GLenum shaderType, String const*  source, char const*  shaderName)
{
	i32 compileResult = 0;
...
inline GLuint CreateShaderProgram(String const*  vertexShaderFilename, String const*  fragmentShaderFilename)
{
	String const*  vertexShaderCode = ReadFile(vertexShaderFilename);

	String const*  fragmentShaderCode = ReadFile(fragmentShaderFilename);
